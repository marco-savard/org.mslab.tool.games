...
public class AquaButton3 extends FocusPanel implements MouseOverHandler, MouseOutHandler {
		...
		if (_btn.isEnabled()) {
			...
			if (_primaryColor != null) {
				String topColor = _primaryColor.toString();
		 		String bottomColor = _primaryColor.brighter(3.0).toString(); 
		 		String patt = "top, {0}, {1}";
	...
	private void refresh() {
		...
		if (_primaryColor != null) {
			...
			//set box shadow
		    patt = "{0} 0px 10px 10px 0px";
			Color shadowColor = brighterColor.blendWith(Color.WHITE, 70);
